...
class TranslationManager {
   ...
   * Lấy phiên âm cho từ
   */
  async getPhoneticTranscription(word, lang = 'en') {
    if (!word || !word.trim()) {
      return { gg: '', us: '', uk: '' };
    }

    try {
      // Sử dụng Free Dictionary API cho tiếng Anh
      if (lang === 'en') {
        return await this.getEnglishPhonetics(word);
      }

      // Cho các ngôn ngữ khác, trả về empty
      return { gg: '', us: '', uk: '' };
    } catch (error) {
      console.error('Phonetic transcription error:', error);
      return {
        gg: '[Loading failed]',
        us: '[Loading failed]',
        uk: '[Loading failed]',
  ...
  /**
   * Lấy phiên âm tiếng Anh từ Free Dictionary API
   */
  async getEnglishPhonetics(word) {
    try {
        ...
        phonetics.forEach((phonetic) => {
          if (phonetic.text) {
            if (phonetic.audio && phonetic.audio.includes('us')) {
              ...
              ggPhonetic = phonetic.text;
            }
          }
        });

        // Fallback nếu không tìm thấy US/UK specific
        if (!usPhonetic && ggPhonetic) usPhonetic = ggPhonetic;
        if (!ukPhonetic && ggPhonetic) ukPhonetic = ggPhonetic;

        return {
          gg: ggPhonetic || '[Not available]',
          us: usPhonetic || '[Not available]',
          uk: ukPhonetic || '[Not available]',
        };
      }

      return { gg: '[Not found]', us: '[Not found]', uk: '[Not found]' };
    } catch (error) {
      console.error('English phonetics error:', error);
      return {
        gg: '[Loading failed]',
        us: '[Loading failed]',
        uk: '[Loading failed]',
